"""Find samples by syn/char instrument / technique / growth window.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Search LiST samples")
    ...
    for row in rows[: args.limit]:
        label = row.get("sampleLabel") or row.get("SampleLabel") or row.get("id")
        print(json.dumps({"label": label, "id": row.get("id") or row.get("ID")}, default=str))
